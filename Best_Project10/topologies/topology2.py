...
class Network_Topology(Topo):
    ...
    def run(self, rate_pkts_s):
        ...
        if not os.path.exists("./data/topology2/"):
            os.mkdir("./data/topology2/")
            os.chmod("./data/topology2/", 775)
        data_dir = "./data/topology2/lambda{}/".format(rate_pkts_s)
        ...
        s1p3 = requests.get("http://0.0.0.0:8080/stats/port/{}/{}".format(int(s1.dpid), 3))
        s3p4 = requests.get("http://0.0.0.0:8080/stats/port/{}/{}".format(int(s3.dpid), 4))

        d = {"s1p2":s1p2, "s2p2":s2p2, "s2p3":s2p3, "s3p3":s3p3, "s1p3":s1p3, "s3p4":s3p4}
...
if __name__=='__main__':
    ...
    try:
        ...
        print("Required arguments: topology2.py <lambda-rate>")
